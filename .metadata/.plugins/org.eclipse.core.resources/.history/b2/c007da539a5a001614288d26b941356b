...
public class source {
	 ...
	 * H[i] i번째 인형의 크기
	 * 
	 * H[i] > H[j]의 경우는 D[i] < D[j] 일 경우 D[j]가 답 아닐 경우 D[i]
	 * H[i] = H[j]의 경우는 Answer는 D[i] > D[j]일 경우 D[i]
	 * H[i] < H[j]의 경우는 담을 수 있는 인형을 증가
	 * 
	 * 
	 * 이 문제는 결국 Longest Increasing Subsequence를 구하는 문제로 계산
	 * 
	 * 가장 긴 증가하는 증가수열을 찾으면 되는 문제 
	 * 
	 * O(n^2)로 풀리는 경우도 있으나 Ranking 정보를 저장하는 Index Tree를 사용하여  O(nlogn)으로 풀린다고 하나 강사 설명 생략!!
	 * 
	 * 
	 * 마트료시카
		문제
